...
try {
    ...
    if ($toolName -in @("write_to_file", "replace_file_content")) {
        ...
        if (-not $fullPath.StartsWith($normalizedWorkspace, [System.StringComparison]::OrdinalIgnoreCase)) {
            [PSCustomObject]@{
                decision = "ask"
                reason = "File edit is outside MealHelper workspace: $targetFile"
        ...
        if ($relPath -match '^\.agents[\\/]') {
            ...
            [PSCustomObject]@{
                decision = "ask"
                reason = "Modifications to agent personas, skills, or configuration files ($relPath) require human approval."
            } | ConvertTo-Json -Compress
            exit 0
        }

        # Normal project files within MealHelper (lib/, test/, android/, README.md, CHANGELOG.md, etc.)
        [PSCustomObject]@{ decision = "allow" } | ConvertTo-Json -Compress
        exit 0
    }
    ...
    if ($toolName -eq "run_command") {
        ...
        if (-not $fullCwd.StartsWith($normalizedWorkspace, [System.StringComparison]::OrdinalIgnoreCase)) {
            [PSCustomObject]@{
                decision = "ask"
                reason = "Command working directory ($cwd) is outside MealHelper workspace."
            } | ConvertTo-Json -Compress
            exit 0
        }
        ...
        # 1. Flutter & Dart commands
        if ($cmd -match '^\s*(flutter|dart)\b') {
            [PSCustomObject]@{ decision = "allow" } | ConvertTo-Json -Compress
            exit 0
        }

        # 2. Gradle / Build commands
        if ($cmd -match '^\s*(\./gradlew|gradlew|gradlew\.bat)\b') {
            [PSCustomObject]@{ decision = "allow" } | ConvertTo-Json -Compress
            exit 0
        }

        # 3. Git safe commands (status, branch, checkout, switch, diff, log, show, add, commit, push, tag, fetch, pull)
        if ($cmd -match '^\s*git\s+(status|branch|checkout|switch|diff|log|show|rev-parse|add|commit|push|tag|fetch|pull)\b') {
            [PSCustomObject]@{ decision = "allow" } | ConvertTo-Json -Compress
            exit 0
        }

        # 4. GitHub CLI safe commands (pr create, pr view, pr list, pr diff, release view, release create, release upload)
        if ($cmd -match '^\s*gh\s+pr\s+(create|view|list|status|diff)\b' -or
            $cmd -match '^\s*gh\s+release\s+(view|create|upload|list)\b') {
            [PSCustomObject]@{ decision = "allow" } | ConvertTo-Json -Compress
            exit 0
        }

        # 5. Safe read/inspection commands (dir, ls, cat, type, echo, pwd, test)
