...
import 'widgets/database_name_section.dart';
...
class _CollaborationScreenState extends ConsumerState<CollaborationScreen> {
  ...
  Future<void> _joinDatabase(String uid) async {
    ...
    try {
      await ref.read(workspaceRepositoryProvider).joinDatabase(uid, code);
  ...
  Future<void> _switchToMyDatabase(String uid) async {
    setState(() => _isLoading = true);
    try {
      await ref.read(workspaceRepositoryProvider).switchActiveDatabase(uid, uid);
