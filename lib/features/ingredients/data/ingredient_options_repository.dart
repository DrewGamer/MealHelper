...
import '../domain/ingredient_options.dart';
...
class IngredientOptionsRepository {
  ...
  DocumentReference _optionsRef(String databaseId) {
    return _firestore
        ...
        .doc(FirestoreDocs.ingredientOptions);
  }
  ...
  Future<void> addProteinSource(String databaseId, String protein) async {
    await _optionsRef(databaseId).set({
      FirestoreFields.proteinSources: FieldValue.arrayUnion([protein])
    }, SetOptions(merge: true));
  }

  Future<void> addIngredient(String databaseId, String ingredient) async {
    await _optionsRef(databaseId).set({
      FirestoreFields.ingredients: FieldValue.arrayUnion([ingredient])
    }, SetOptions(merge: true));
  }
  ...
  Future<void> removeProteinSource(String databaseId, String protein) async {
    ...
    batch.set(_optionsRef(databaseId), {
      FirestoreFields.proteinSources: FieldValue.arrayRemove([protein])
    }, SetOptions(merge: true));
    ...
    final mealsQuery = await _firestore
        ...
        .where(FirestoreFields.proteinSource, isEqualTo: protein)
        .get();

    for (var doc in mealsQuery.docs) {
      batch.update(doc.reference, {FirestoreFields.proteinSource: null});
    }
  ...
  Future<void> removeIngredient(String databaseId, String ingredient) async {
    ...
    // Remove from options
    batch.set(_optionsRef(databaseId), {
      FirestoreFields.ingredients: FieldValue.arrayRemove([ingredient])
    }, SetOptions(merge: true));
    ...
    final mealsQuery = await _firestore
        ...
        .doc(databaseId)
        .collection(FirestoreCollections.meals)
        .where(FirestoreFields.ingredients, arrayContains: ingredient)
        .get();
  ...
  Future<void> renameProteinSource(String databaseId, String oldProtein, String newProtein) async {
    ...
    batch.set(_optionsRef(databaseId), {
      FirestoreFields.proteinSources: FieldValue.arrayRemove([oldProtein])
    }, SetOptions(merge: true));
    batch.set(_optionsRef(databaseId), {
      FirestoreFields.proteinSources: FieldValue.arrayUnion([newProtein])
    }, SetOptions(merge: true));
    ...
    final mealsQuery = await _firestore
        ...
        .doc(databaseId)
        .collection(FirestoreCollections.meals)
        .where(FirestoreFields.proteinSource, isEqualTo: oldProtein)
        .get();

    for (var doc in mealsQuery.docs) {
      batch.update(doc.reference, {FirestoreFields.proteinSource: newProtein});
    }
  ...
  Future<void> renameIngredient(String databaseId, String oldIngredient, String newIngredient) async {
    ...
    // Update options (remove old, add new)
    batch.set(_optionsRef(databaseId), {
      FirestoreFields.ingredients: FieldValue.arrayRemove([oldIngredient])
    }, SetOptions(merge: true));
    batch.set(_optionsRef(databaseId), {
      FirestoreFields.ingredients: FieldValue.arrayUnion([newIngredient])
    }, SetOptions(merge: true));
    ...
    final mealsQuery = await _firestore
        ...
        .doc(databaseId)
        .collection(FirestoreCollections.meals)
        .where(FirestoreFields.ingredients, arrayContains: oldIngredient)
        .get();
    ...
    for (var doc in mealsQuery.docs) {
      ...
      final data = doc.data();
      List<String> ingredients = List<String>.from(data[FirestoreFields.ingredients] ?? []);
      if (ingredients.contains(oldIngredient)) {
        ingredients.remove(oldIngredient);
        if (!ingredients.contains(newIngredient)) {
          ingredients.add(newIngredient);
        }
        batch.update(doc.reference, {FirestoreFields.ingredients: ingredients});
      }
    }
