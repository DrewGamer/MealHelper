...
import '../../auth/presentation/widgets/email_password_dialog.dart';
...
class _SettingsScreenState extends ConsumerState<SettingsScreen> {
  ...
  Future<void> _handleGoogleFlow(bool isLoginIntent) async {
    ...
    try {
      final credential = await authRepo.getGoogleCredential();
      if (credential == null) {
      ...
      if (isLoginIntent) {
        ...
        try {
          await authRepo.linkWithGoogleCredential(credential);
  ...
  Future<void> _handleEmailFlow(bool isLoginIntent) async {
    ...
    final result = await EmailPasswordDialog.show(
      context,
      title: title,
      actionText: action,
    );
    ...
    try {
      if (isLoginIntent) {
        // Direct Login Intent - Optimistic Link
        try {
          await authRepo.linkWithEmailAndPassword(email, password);
          ...
          if (choice == 'keep') {
            ...
            // Cancel -> undo the creation
            if (mounted) setState(() => _isLoading = true);
            await authRepo.unlinkProvider('password');
            if (mounted) _showSuccess('Cancelled. You are still a guest.');
          }
        } on FirebaseAuthException catch (e) {
